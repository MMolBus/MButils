...
#' @param only_directories A logical value indicating whether to include only 
#' directories (TRUE) or both files and directories (FALSE). Default is TRUE.
#' @return A Node object representing the directory tree.
#' @examples
#' \dontrun{
#' tree <- directorytree("/path/to/directory", only_directories = TRUE)
#' print(tree)
#' }
#' @export
directorytree <- function(path, only_directories = TRUE) {
  ...
  for (tip in tips) {
    ...
    parts <- strsplit(tip, .Platform$file.sep)[[1]]
